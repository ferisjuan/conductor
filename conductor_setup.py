...
def prompt_for_credentials(existing_config=None, existing_token=None):
    ...
    if (
        ...
        and questionary.confirm(
            f"Use existing company '{company_name}'?", default=True
        ).ask()
    ):
        ...
        company_name = questionary.text(
            ...
            or "Company name cannot be empty",
        ).ask()
        if not company_name:
    ...
    if (
        username
        and questionary.confirm(
            f"Use existing username '{username}'?", default=True
        ).ask()
    ):
        pass
    else:
        username = questionary.text(
            ...
            validate=lambda text: ("@" in text and len(text.strip()) > 0)
            or "Must be a valid email address",
        ).ask()
        if not username:
            print("Setup cancelled.")
            sys.exit(0)
    ...
    if (
        api_token
        and questionary.confirm("Use existing API token (masked)?", default=True).ask()
    ):
        pass
    else:
        print(
...
def select_projects(projects, existing_projects=None):
    ...
    choices = [
        questionary.Choice(
            ...
            value=p.key,
            checked=(existing_projects and p.key in existing_projects),
        )
        for p in projects
    ]
...
def select_statuses(statuses, existing_statuses=None):
    ...
    choices = [
        questionary.Choice(
            title=s, value=s, checked=(existing_statuses and s in existing_statuses)
        )
        for s in statuses
    ]
...
def ask_branch_prefixes(existing_config=None):
    ...
    use_prefixes = questionary.confirm(
        "Use branch prefixes (recommended for clarity)?", default=default_value
    ).ask()
...
def main():
    ...
    server, username, api_token = prompt_for_credentials(
        existing_config, existing_token
    )

    # Save credentials immediately
    print("\n💾 Saving credentials...")
    save_config_partial({"jira_server": server, "jira_username": username})
    ...
    print(
        f"   Projects:         {', '.join(selected_projects) if selected_projects else 'All projects'}"
    )
    print(
        f"   Statuses:         {', '.join(selected_statuses) if selected_statuses else 'All statuses'}"
    )
    print(
        f"   Branch prefixes:  {'✅ Enabled' if use_branch_prefixes else '❌ Disabled'}"
    )
